...
Astro 정적 사이트입니다. 메인은 공개 대상 앱을 파스텔 카드와 실제 화면으로 소개합니다. 새 앱은 JSON과 Markdown으로 등록하며 기존 앱별 랜딩·문서·직접 주소는 유지합니다.

## 메인 리뉴얼 (2026-09-09)

3번 Playful Collection 디자인과 가벼운 애니메이션을 적용했습니다. 종료 서비스는 메인에서 제외했습니다. 새 앱 등록, 움직임 설정, 미리보기와 검증은 [메인 리뉴얼 안내](docs/HOME_PLAYFUL.md)를 참고하세요.

문서 승인 관련 아래 설명은 초기 작업 이력입니다. 현재 공개 여부는 콘텐츠와 `npm run release:check` 결과를 기준으로 확인하세요.

## 무브컷 랜딩 페이지 (2026-09-09)

`/movecut/` 소개 페이지를 추가했습니다. 기존 `/movecut/privacy/`, `/movecut/terms/`와 버전별 문서를 유지하며 홈 앱 목록과 사이트맵에서 연결됩니다. 아이보리·코랄 디자인, 대표 카드 3종, 홍보 이미지, 라이트·다크 앱 화면 미리보기를 제공합니다. 스토어 URL은 아직 등록되지 않아 공개 준비 중으로 표시합니다. 구현·검증·스토어 연결 방법은 [무브컷 랜딩 안내](docs/MOVECUT_LANDING.md)를 참고하세요. 아래 메코디 문서 관련 내용은 초기 작업 이력이며 현재 공개 여부는 콘텐츠 frontmatter와 `release:check` 결과를 기준으로 확인하세요.
