...
| 분석·오류 보고 | Analytics는 최초 실행 시 비활성화 상태로 시작하고 이용자 동의 후에만 이벤트를 전송합니다. 설정에서 철회할 수 있으며, Crashlytics 패키지는 포함되어 있어 오류 보고를 별도 항목으로 다룹니다. |
...
4. 최종 배포 빌드에서 Crashlytics·AdMob·UMP가 처리하는 실제 항목, 만 13세 이상 대상 설정 및 필요한 아동·법정대리인 절차.
5. Google Analytics·외부 서비스의 국외 이전 세부사항을 실제 계약·콘솔 설정과 대조.
6. 최종 승인자, 실제 시행일, 구버전 이용자에게 안내하는 방식.
