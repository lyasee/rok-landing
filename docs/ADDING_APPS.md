...
홈은 레지스트리의 `visible: true` 앱만 `order` 순서로 자동 연결합니다. 기존 `src/data/products.ts`는 메인에서 사용하지 않습니다. 새 앱은 공통 대표 화면 구성이 적용되며, 선택 항목 `home`으로 색상과 메인용 문구를 설정할 수 있습니다. 지원 항목과 예시는 [메인 리뉴얼 안내](HOME_PLAYFUL.md)를 참고하세요. 위고는 직접 주소만 유지하고 메인에는 노출하지 않으며 같은 slug를 다시 등록하지 않습니다.
